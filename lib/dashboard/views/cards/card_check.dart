...
/// - Grid مرن childAspectRatio محسوب حسب العرض
/// - حد أدنى آمن لارتفاع البلاطة لمنع أي Overflow
/// - تمرير تلقائي عند ضيق الارتفاع (لا نقفل السكروول)
...
class _DashboardRootState extends State<DashboardRoot>
  ...
  Widget build(BuildContext context) {
    // نحدّ سقف تكبير النص لتفادي انفجار الواجهة على بعض الأجهزة
    ...
    return MediaQuery(
      ...
      child: Directionality(
        ...
        child: Scaffold(
          ...
          // نسمح بتحريك المحتوى لأعلى عند ظهور لوحة المفاتيح
                  ...
                  child: Padding(
                    ...
                    child: LayoutBuilder(
                      builder: (context, c) {
                        ...
                        // نحسب عرض البلاطة الفعلي، ونستهدف ارتفاعًا آمِنًا
                        final tileWidth =
                            (w - spacing * (cross - 1)) / cross;
                        // حد أدنى آمن للارتفاع لتجنّب أي Overflow
                        const minTileH = 120.0;
                        // نضبط نسبة العرض للارتفاع = العرض / الارتفاع المستهدف
                        // نضيف هامشًا بسيطًا للنص/السكيل
                        final targetH = math.max(
                          minTileH,
                          130.0 * clampedTextScale,
                        );
                        final aspect = tileWidth / targetH;

                        return GridView.builder(
                          // نتركه يتمرّر عند الحاجة (لا تقفل التمرير)
                          ...
                          itemBuilder: (ctx, i) {
                            ...
                            // دخول متدرّج + Padding متحرك خفيف
      ...
      child: GestureDetector(
        ...
        child: AnimatedScale(
          ...
          child: Container(
            ...
            // ضمان حد أدنى للارتفاع لتفادي أي Overflow حتى لو حصل خطأ حسابي
            child: ConstrainedBox(
              constraints: const BoxConstraints(minHeight: 120),
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                mainAxisSize: MainAxisSize.min, // ما نطلبش ارتفاع زيادة
                children: [
                  Icon(
                    widget.icon,
                    size: 52,
                    color: Colors.white,
                    // خصائص Material Symbols (لو الأيقونة من الباكدج)
                    fill: 1.0,
                    weight: 650,
                    grade: 100,
                    opticalSize: 48,
